## Hi, I’m Ilias 👋

Software engineer with 20+ years of experience building distributed systems,
cloud-native applications, and blockchain solutions.
I work primarily in Java, Kotlin, and Go — and spend time on Kubernetes, Azure, and Hyperledger.

---

## Tech Stack

![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=flat&logo=kotlin&logoColor=white)
![Go](https://img.shields.io/badge/Go-00ADD8?style=flat&logo=go&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white)
![Helm](https://img.shields.io/badge/Helm-0F1689?style=flat&logo=helm&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat&logo=microsoftazure&logoColor=white)
![Hyperledger](https://img.shields.io/badge/Hyperledger-2F3134?style=flat&logo=hyperledger&logoColor=white)
![OpenSCAD](https://img.shields.io/badge/OpenSCAD-4B9CD3?style=flat)

---

## GitHub Stats

![Ilias's GitHub stats](https://github-readme-stats.vercel.app/api?username=istergiou&show_icons=true&theme=default&hide_border=true)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=istergiou&layout=compact&hide_border=true)

![GitHub Streak](https://streak-stats.demolab.com?user=istergiou&theme=default&hide_border=true)


---

## Areas of Focus

- Distributed systems and microservices (Java, Kotlin, Go)
- Cloud-native infrastructure (Kubernetes, Helm, Azure)
- Blockchain / permissioned ledgers (Hyperledger Fabric)
- 3D parametric modelling (OpenSCAD)

---

## Resources

- [AI tools & services](resources-ai.md) — curated list of AI tools by use case
- [Git cheatsheet](git-cheatsheet.md) — quick reference for everyday git operations

---

## Connect

- Email: ilias.stergiou at gmail.com
- LinkedIn: *(add your profile URL)*
- Personal site / blog: *(if applicable)*


<!--
**istergiou/istergiou** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
